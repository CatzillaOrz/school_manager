
'use strict';

var _ = require('lodash'),
   NewsService = require('../../services/newsService');

module.exports = {
    getNewsList : function (req, res) {
       NewsService.getNewsListSync(req.query, req.user.access_token)
            .then(function (data) {
                res.json(data);
            })
            .catch(function (e) {
                res.status(e.code).send(e.message);
            })
    },
    addNews:function (req,res) {
       NewsService.addNewsSync(req.body, req.user.access_token)
            .then(function (data) {
                res.json(data);
            })
            .catch(function (e) {
                res.status(e.code).send(e.message);
            })
    },
    deleteNews:function (req,res) {
       NewsService.deleteNewsSync(req.query, req.user.access_token)
            .then(function (data) {
                res.json(data);
            })
            .catch(function (e) {
                res.status(e.code).send(e.message);
            })
    },
    updateNews:function (req,res) {
       NewsService.updateNewsSync(req.body, req.user.access_token)
            .then(function (data) {
                res.json(data);
            })
            .catch(function (e) {
                res.status(e.code).send(e.message);
            })
    },
    getNewsById: function (req, res) {
       NewsService.getNewsByIdSync(req.query)
            .then(function (data) {
                res.json(data);
            })
            .catch(function (e) {
                res.status(e.code).send(e.message);
            })
    },
    publishNews:function (req,res) {
        NewsService.publishNewsSync(req.body, req.user.access_token)
            .then(function (data) {
                res.json(data);
            })
            .catch(function (e) {
                res.status(e.code).send(e.message);
            })
    },
    getNewsListByOrg : function (req, res) {
        NewsService.getNewsListByOrgSync(req.query)
            .then(function (data) {
                res.json(data);
            })
            .catch(function (e) {
                res.status(e.code).send(e.message);
            })
    },

};



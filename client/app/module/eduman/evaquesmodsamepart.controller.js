...
angular.module('dleduWebApp')
	.controller('EvaQuesModSamePartCtrl', function ($scope, $state, $timeout, AuthService, EduManService, messageService,
	                                        $filter, ngDialog) {
		$scope.evaQuesModFn={
			...
			saveQues: function(){
				...
				if($state.params.id){
					...
					cloneParams.endDate = cloneParams.endDate + ' 23:59:59';
					this.convertRadio(cloneParams);
					cloneParams.quesType = 30; //30同行评教
			...
			getEvaQuesList: function () {
				...
				EduManService.getEvaQuesList(params).$promise
					.then(function (data) {
						...
						if(!data.data){
							messageService.openMsg("还没有问卷，请先填写!");
						}else{
							ngDialog.open({

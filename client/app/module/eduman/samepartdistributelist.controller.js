...
angular.module('dleduWebApp')
	.controller('SamePartDistribListCtrl', function ($scope, $state, $timeout, AuthService, EduManService, messageService,
		...
		$scope.distributeListFn = {
			...
			getEvaQuesUnDist: function () {
				...
				EduManService.getSamePartList(params).$promise
					.then(function (data) {
						that.records = data.data;
						//增加check属性
						that.addCheckProperty(that.records);
			...
			saveWeight: function(){
				...
				EduManService.saveWeight(params).$promise
					.then(function (data) {
						messageService.openMsg("权重设置成功！");
						that.selDistObj = [];
						that.isSetWeight = false;
						that.findByOption('complete');
					})
					.catch(function (error) {
						messageService.openMsg("权重设置失败！");
					})
			},

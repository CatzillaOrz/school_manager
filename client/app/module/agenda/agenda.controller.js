    ...
    .controller('AgendaCtrl', function ($scope, $state, $timeout, $document, AuthService, SchoolYearService, ngDialog, $compile, messageService, csCalendarConfig, TeachClassService) {
        ...
        $scope.schedule = {
                        ...
                        if(obj.teachingClassId == courseCard.teachingClassId){
                            angular.forEach(obj, function (item, index) {
                                if (item._id == courseCard._id) {
                                    ...
                                    //修改周末不显示的bug,当星期天时将返回的一周第几天改为7
                                    var whichDay = !parseInt(w) ? 7 : parseInt(w);
                                    item.start = new Date(y, m, parseInt(d) - whichDay + parseInt(item.dayOfWeek), parseInt(item.periodMo - 1), 0);
                                    item.end = new Date(y, m, parseInt(d) - whichDay + parseInt(item.dayOfWeek), parseInt(item.periodMo - 1) + parseInt(item.periodNum), 0);
                                }
                            });
                        }
                    });
            ...
            renderSource: function (obj, arr) {
                ...
                //修改周末不显示的bug,当星期天时将返回的一周第几天改为7
                var whichDay = !parseInt(w) ? 7 : parseInt(w);
                obj.start = new Date(y, m, parseInt(d) - whichDay + parseInt(obj.dayOfWeek), parseInt(obj.periodMo - 1), 0);
                obj.end = new Date(y, m, parseInt(d) - whichDay + parseInt(obj.dayOfWeek), parseInt(obj.periodMo - 1) + parseInt(obj.periodNum), 0);
                arr.push(obj);
                // console.log(arr);
            },
            ...
            toEditCourse: function (classId, courseId, jsEvent) {
                ...
                _this.courseCard.endWeek = _this.courseCard.endWeekNo != 1 ?  _this.teachWeekListB[_this.courseCard.endWeekNo - 1] : _this.teachWeekListB[0];
                ngDialog.open({
